...
def get_user_input() -> dict:
    """交互式获取用户输入"""
    print("\n请输入原始请求参数（包含URL参数、headers 和 data；可直接从抓包复制）：")
    print("示例请求URL开头:")
    ...
    if not info.get("utdid"):
        ...
        print("1. 手动输入utdid")
        print("2. 返回并重新输入请求")
        choice_utdid = input("请选择 (1/2): ").strip()
        if choice_utdid == "1":
            manual_utdid = input("请输入utdid: ").strip()
            if manual_utdid:
                info["utdid"] = manual_utdid
            else:
                print("错误：utdid不能为空")
                sys.exit(1)
        elif choice_utdid == "2":
...
def upload_bytes(file_name: str, file_bytes: bytes, mime: str, auth_info: dict) -> str:
    ...
    if "text/html" in content_type or response.text.lstrip().startswith("<!DOCTYPE html"):
        ...
        raise RuntimeError(
            "Upload response looks like an HTML page (可能触发风控/需要验证)。"
            f" status={response.status_code} content_type={content_type} body_snippet={snippet}"
        )
    try:
        body = response.json()
    except ValueError:
        snippet = response.text[:500].strip()
        raise RuntimeError(f"Upload response is not JSON. status={response.status_code} body_snippet={snippet}")
